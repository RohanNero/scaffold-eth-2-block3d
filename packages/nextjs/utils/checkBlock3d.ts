import { Block3dConfig, UserData } from "../types/block3d";
import { checkIsRoutePublic } from "./checkIsRoutePublic";
import { checkNftRules } from "./checkNftRules";
import { checkSimpleRules } from "./checkSimpleRules";
import { checkTokenRules } from "./checkTokenRules";
...
export const checkBlock3d = async (
  ...
  const { simplePassing, simpleFailing } = await checkSimpleRules(address, block3dConfig);
  const { tokenPassing, tokenFailing } = await checkTokenRules(address, block3dConfig, config);
  const { nftPassing, nftFailing } = await checkNftRules(address, block3dConfig, config);

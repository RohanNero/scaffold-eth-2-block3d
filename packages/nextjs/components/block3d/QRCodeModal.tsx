...
export function QRCodeModal({ qrModalVisible, setQrModalVisible, address }: QRCodeModalProps) {
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center space-y-6 pt-6">
        ...
        {/* Address display */}
        <div className="!rounded-xl flex gap-3 relative group hidden lg:block" onClick={() => copyToClipboard(address)}>
          <div className="cursor-pointer flex items-center font-medium bg-white rounded-lg text-lg p-2">{address}</div>
          <span className="absolute bottom-10 left-1/2 transform -translate-x-1/2 px-2 py-1 bg-blue-300 whitespace-nowrap p-4 text-white text-base rounded shadow-lg pointer-events-none opacity-0 transition-opacity duration-300 group-hover:opacity-100 z-20">
            {addressCopied == true ? "Address Copied!" : "Click to copy address"}
          </span>
        </div>

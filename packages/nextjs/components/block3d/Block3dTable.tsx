...
export function Block3dTable({ userData }: { userData: UserData | undefined }) {
  ...
  const hasTokenOrNftRules = (userData?.token?.failing?.length ?? 0) > 0 || (userData?.nft?.failing?.length ?? 0) > 0;
  ...
  const getRows = () => {
    ...
    allFailingRules.forEach((rule, index) => {
      rows.push({
        ...
        type: rule.type == "nft" ? rule.type.toUpperCase() : rule.type.charAt(0).toUpperCase() + rule.type.slice(1),
      });
    });
    return rows;
  };
  ...
  return (
    <Table className="w-1/2 text-2xl bg-blue-300 rounded-lg hidden lg:block" aria-label="Blocked reasons table">
